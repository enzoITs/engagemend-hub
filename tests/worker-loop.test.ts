...
beforeEach(async () => { await prisma.memberEvent.deleteMany(); await prisma.community.deleteMany(); await prisma.job.deleteMany(); await prisma.user.deleteMany(); vi.mocked(runYoutubeSync).mockClear(); vi.mocked(runDiscordBackfill).mockClear(); vi.useFakeTimers(); });
