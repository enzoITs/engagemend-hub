...
engajamento via Groq (LLM open-source), armazena no ChromaDB, e exporta pro
schema universal de eventos que o motor TypeScript (engagemend-discord)
ingere via `pnpm ingest:youtube`.
...
from export_events import export_universal_events
...
def main():
    ...
    # Etapa 3: armazenamento no ChromaDB (busca semantica, opcional)
    print("\n=== Armazenando no ChromaDB ===")
    store_comments(classified)

    # Etapa 4: export pro schema universal de eventos (motor TS le esse arquivo)
    print("\n=== Exportando eventos pro motor de engajamento ===")
    export_universal_events(classified)
